...
from (select visited_on,sum(amount) as amount from Customer group by visited_on) as C1 join  (select visited_on,sum(amount) as amount from Customer group by visited_on) as C2
